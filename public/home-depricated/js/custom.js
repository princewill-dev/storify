...
var MoonCart = function(){
	...
	var handleOpenModal = function(){
		// Check if modal should be shown based on frequency setting
		var modalFrequency = window.greetingModalFrequency || 'always';
		var cookieName = 'greeting_modal_shown';
		...
		switch(modalFrequency) {
			...
			case 'once_per_session':
				shouldShow = getCookie(cookieName) != 'true';
				break;
			case 'once_per_day':
				shouldShow = getCookie(cookieName) != 'true';
				break;
		...
		if(shouldShow && window.greetingModalContent){
			jQuery('body').append(window.greetingModalContent);
			setTimeout(function() {
				jQuery("#greetingModal").modal('show');
			}, 2000);
		}
	}
	...
	var handleOpenModalCookie = function(){
		$(document).on('click','#greetingModal .btn-close',function(){
			var modalFrequency = window.greetingModalFrequency || 'always';
			var cookieName = 'greeting_modal_shown';
			var expiryHours = 0;
			
			// Set cookie expiry based on frequency
			switch(modalFrequency) {
				case 'once_per_session':
					...
					break;
				case 'once_per_day':
					expiryHours = 24 * 60; // 24 hours in minutes
					setCookie(cookieName, 'true', expiryHours);
					break;
				case 'once_per_week':
					expiryHours = 7 * 24 * 60; // 7 days in minutes
					setCookie(cookieName, 'true', expiryHours);
					break;
				case 'once_per_month':
					expiryHours = 30 * 24 * 60; // 30 days in minutes
					setCookie(cookieName, 'true', expiryHours);
					break;
			}
		});
		
		// Legacy support for old modal
