...
use App\Http\Controllers\Checkout\PaymentMethodController;
...
Route::domain(config('app.main_domain', parse_url(config('app.url'), PHP_URL_HOST)))->group(function () {
    ...
    Route::get('/p/{code}', function ($code) {
        $product = Product::with('store')->where('product_code', $code)->firstOrFail();
...
if (config('app.env') === 'local') {
    Route::prefix('{store_subdomain}')
        ...
        ->group(function () {
            ...
            // Checkout
            Route::get('/checkout/{token}', [CheckoutPageController::class, 'checkout'])->name('local.checkout.index');
            Route::post('/checkout', PlaceOrderController::class)->name('local.checkout.process');
            Route::get('/checkout/payment/{order}', [CheckoutPageController::class, 'payment'])->name('local.checkout.payment');
            Route::get('/checkout/{order}/payment-methods', [PaymentMethodController::class, 'show'])->name('local.checkout.payment-methods');
            Route::post('/checkout/{order}/payment-methods', [PaymentMethodController::class, 'select'])->name('local.checkout.payment-methods.select');
            Route::post('/checkout/save-address', [DeliveryAddressController::class, 'store'])->name('local.checkout.save-address');
            Route::get('/checkout/{order}/bank-transfer', [BankTransferController::class, 'show'])->name('local.payment.bank-transfer');
            Route::post('/checkout/{order}/bank-transfer/confirm', [BankTransferController::class, 'confirmPayment'])->name('local.payment.bank-transfer.confirm');
            Route::get('/checkout/{order}/payment-pending', [BankTransferController::class, 'pending'])->name('local.payment.pending');
...
Route::domain('{store_subdomain}.'.config('app.main_domain', parse_url(config('app.url'), PHP_URL_HOST)))
    ...
    ->group(function () {
        ...
        // Checkout
        Route::get('/checkout/{token}', [CheckoutPageController::class, 'checkout'])->name('checkout.index');
        Route::post('/checkout', PlaceOrderController::class)->name('checkout.process');
        Route::get('/checkout/payment/{order}', [CheckoutPageController::class, 'payment'])->name('checkout.payment');

        // Payment Method Selection
        Route::get('/checkout/{order}/payment-methods', [PaymentMethodController::class, 'show'])->name('checkout.payment-methods');
        Route::post('/checkout/{order}/payment-methods', [PaymentMethodController::class, 'select'])->name('checkout.payment-methods.select');
        Route::post('/checkout/save-address', [DeliveryAddressController::class, 'store'])->name('checkout.save-address');

        // Bank Transfer Payment
        Route::get('/checkout/{order}/bank-transfer', [BankTransferController::class, 'show'])->name('payment.bank-transfer');
        Route::post('/checkout/{order}/bank-transfer/confirm', [BankTransferController::class, 'confirmPayment'])->name('payment.bank-transfer.confirm');
        Route::get('/checkout/{order}/payment-pending', [BankTransferController::class, 'pending'])->name('payment.pending');
        Route::get('/checkout/{order}/payment-remaining', [BankTransferController::class, 'remaining'])->name('payment.remaining');
    });
