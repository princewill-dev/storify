...
<!-- @if($pageStyling && $pageStyling->background_color)
    <style>
        body {
            background-color: {{ $pageStyling->background_color }} !important;
        }
        @if($pageStyling->custom_css)
        {!! $pageStyling->custom_css !!}
        @endif
    </style>
@endif -->
...
<div class="page-content" style="background-color: {{ $pageStyling->background_color ?? '#ffffff' }} !important;">
		
    <div class="d-sm-flex justify-content-between container-fluid py-3">
        <nav aria-label="breadcrumb" class="breadcrumb-row">
            <ul class="breadcrumb mb-0">
                <li class="breadcrumb-item"><a href="{{ route('home.store.products.index', ['store_slug' => $product->store->slug]) }}">{{ $product->store->name }}</a></li>
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-xl-4 col-md-4">
                    <div class="dz-product-detail sticky-top" style="background-color: {{ $pageStyling->background_color ?? '#ffffff' }} !important;">
                                        ...
                                        @foreach($galleryItems as $gi)
                                            <div class="swiper-slide">
                                                <div class="dz-media DZoomImage">
                                                    <a class="mfp-link lg-item" href="{{ $gi['full'] }}" data-src="{{ $gi['full'] }}">
                                                        <i class="fa fa-search-plus dz-maximize top-left"></i>
                                                    </a>
                                                    <img src="{{ $gi['full'] }}" alt="{{ $product->name }}">
                                                </div>
                                            </div>
                                        @endforeach
                                    @else
                                        <div class="swiper-slide">
                                            <div class="dz-media DZoomImage">
                                                <a class="mfp-link lg-item" href="{{ $placeholder }}" data-src="{{ $placeholder }}">
                                                    <i class="fa fa-search-plus dz-maximize top-left"></i>
                                                </a>
                            ...
                            <div class="swiper product-gallery-swiper thumb-swiper-lg">
                                <div class="swiper-wrapper">
                                    @if(($galleryItems ?? collect())->count())
                                        @foreach($galleryItems as $gi)
                                            <div class="swiper-slide">
                                                <img src="{{ $gi['thumb'] }}" alt="{{ $product->name }}">
                                            </div>
                                        @endforeach
                                    @else
                                        <div class="swiper-slide">
                ...
                <div class="col-xl-8 col-md-8">
                    <div class="row">
                        <div class="col-xl-7">
                            <div class="dz-product-detail style-2 p-t20 ps-0" style="background-color: {{ $pageStyling->background_color ?? '#ffffff' }} !important;">
                                <div class="dz-content">
                                    <div class="dz-content-footer">
                                        <div class="dz-content-start">
                                            <!-- <span class="badge bg-purple mb-2">{{ strtoupper($product->status ?? 'active') }}</span> -->
                                    ...
                                    <div class="meta-content m-b20 d-flex align-items-end">
                                        <div class="me-3">
                                            ...
                                            <span id="product-price" class="price-num d-block">
                                                @if(!empty($product->has_variants))
                                                    {{ $priceInfoSymbol ?? '—' }}
                                                @else
                                                    @if(!empty($hasDiscount))
                                                        <span class="d-block">{{ $displayDiscountedAmount }}</span>
                                                        <small class="text-muted d-block"><span class="text-decoration-line-through">{{ $displayBaseAmount }}</span></small>
                                                    @else
                                                        {{ $displayBaseAmount }}
                                                    @endif
                                                @endif
                                    ...
                                    <div class="d-flex align-items-center gap-3 mb-3">
                                        <span class="fw-bold">Buy Option:</span>
                                        <div class="btn-group" role="group" aria-label="Buy Option">
                                            <input type="radio" class="btn-check" name="buy_option" id="buy_single" autocomplete="off" checked>
                                            <label class="btn btn-outline-primary btn-sm" for="buy_single">Single</label>

                                            <input type="radio" class="btn-check" name="buy_option" id="buy_bulk" autocomplete="off">
                                            <label class="btn btn-outline-primary btn-sm" for="buy_bulk">
                                                Bulk (Min {{ $product->bulk_quantity }})
                                            </label>
                                        </div>
                                    </div>
                                    @endif
                                            ...
                                            <div class="d-block mt-3">
                                                <div class="row g-3 align-items-end">
                                                    <div class="col-12 col-md-3">
                                                        ...
                                                        <input id="qty-lg" name="qty" type="number" min="1" value="1" class="form-control">
                                                    </div>
                                                    ...
                                                    <div class="col-12 col-md-3">
                                                        ...
                                                        <select id="size-select" class="form-select">
                                                            @foreach($sizeOptions as $val => $label)
                                                                <option value="{{ $val }}" @selected((string)($defaultSelection['size'] ?? '') === (string)$val)>{{ $label }}</option>
                                                            @endforeach
                                                        </select>
                                                    </div>
                                                    @endif
                                                    ...
                                                    <div class="col-12 col-md-3">
                                                        ...
                                                        <select id="weight-select" class="form-select">
                                                            @foreach($weightOptions as $val => $label)
                                                                <option value="{{ $val }}" @selected((string)($defaultSelection['weight'] ?? '') === (string)$val)>{{ $label }}</option>
                                                            @endforeach
                                                        </select>
                                                    </div>
                                                    @endif
                                                    ...
                                                    <div class="col-12 col-md-3">
                                                        ...
                                                        <select id="color-select" class="form-select">
                                                            @foreach($colorOptions as $val => $label)
                                                                <option value="{{ $val }}" @selected((string)($defaultSelection['color'] ?? '') === (string)$val)>{{ $label }}</option>
                                                            @endforeach
                                                        </select>
                                                    </div>
                                                    @endif
                                                </div>
                                                <small id="variant-stock" class="text-muted d-block mt-2"></small>
                                            </div>
                                        @else
                                            <!-- <div class="product-num">
                                                <div class="btn-quantity quantity-sm light d-xl-none d-block d-sm-block">
                                                    <label class="form-label">Quantity</label>
                                                    <input id="qty-single" type="text" value="1" name="qty_single" data-max="{{ max((int)($baseMeta['qty'] ?? 0), 1) }}">
                                                </div>
                                                <div class="btn-quantity light d-xl-block d-sm-none d-none">
                                                    <label class="form-label">Quantity</label>
                                                    <input id="qty-single-desktop" type="text" value="1" name="qty_single_desktop" data-max="{{ max((int)($baseMeta['qty'] ?? 0), 1) }}">
                                                </div>
                                                <div class="d-block mt-2">
                                                    <label class="form-label">Size</label>
                                                    <div class="btn-group product-size mb-0">
                                                        @php($sizeLabel = $baseMeta['size'] ?? null)
                                                        <input type="radio" class="btn-check" name="size_single" id="size_single_1" checked>
                                                        <label class="btn btn-light" for="size_single_1">{{ $sizeLabel ?: '—' }}</label>
                                                    </div>
                                                </div>
                                                <div class="meta-content mt-2">
                                                    <label class="form-label">Color</label>
                                                    <div class="d-flex align-items-center color-filter">
                                                        <div class="form-check">
                                                            <input class="form-check-input" type="radio" name="color_single" id="color_single_1" aria-label="..." checked>
                                                            <span style="background: {{ $baseMeta['color'] ?? '#C7D1CF' }}"></span>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div> -->
                                            ...
                                            <div class="row g-3 align-items-end">
                                                <div class="col-12 col-md-4">
                                                    ...
                                                    <input id="qty-single" name="qty" type="number" class="form-control form-control-sm" min="1" max="{{ max((int)($baseMeta['qty'] ?? 0), 1) }}" value="1" />
                                                </div>
                                                <div class="col-12 col-md-4">
                                                    <label class="form-label mb-1">Size</label>
                                                    <input type="text" class="form-control form-control-sm" value="{{ $baseMeta['size'] ?? '' }}" placeholder="—" readonly />
                                                </div>
                                                <div class="col-12 col-md-4">
                                                    <label class="form-label mb-1">Color</label>
                                                    <input type="text" class="form-control form-control-sm text-uppercase" value="{{ $baseMeta['color'] ?? '' }}" placeholder="—" readonly />
                                                </div>
                                            </div>
                                        @endif
                                    </div>
                                    ...
                                    <div class="dz-info">
                                        <ul>
                                            ...
                                            <li>
                                                <strong>Tags:</strong>
                                                @php($tags = array_filter(array_map('trim', explode(',', (string)($product->tags ?? '')))))
                                                @if(!empty($tags))
                                                    @foreach($tags as $i => $t)
                                                        <span>{{ $t }}@if($i < count($tags)-1),@endif</span>
                                                    @endforeach
                                                @else
                                                    <span>—</span>
                                                @endif
                        ...
                        <div class="col-xl-5">
                            <div class="cart-detail" style="background-color: {{ $pageStyling->background_color ?? '#ffffff' }} !important;">
                                @if(!empty($hasDiscount))
                                <div class="w-100 m-b20">
                                    <div class="alert alert-success d-flex align-items-center justify-content-between mb-0" role="alert">
                                        <div class="d-flex align-items-center gap-2">
                                            <span class="badge bg-white text-success border">-{{ $displayDiscountPct }}%</span>
                                            <strong>Get {{ $displayDiscountPct }}% Discount</strong>
                                        </div>
                                ...
                                <div class="card m-b15">
                                    <div class="card-body">
                                        <div class="d-flex align-items-center gap-3 m-b15">
                                            @if(!empty($product->store->logo_path))
                                                <img src="{{ asset('storage/'.$product->store->logo_path) }}" alt="{{ $product->store->name }}" style="width:48px;height:48px;object-fit:cover;border-radius:8px;">
                                            @endif
                                            <div class="d-flex flex-column">
                                                <span class="text-muted small">Sold by</span>
                                                <h6 class="mb-0">{{ $product->store->name }}</h6>
                                            </div>
                                        </div>
                                        <ul class="list-unstyled mb-2">
                                            @if(!empty($product->store->support_email))
                                                <li class="mb-1"><i class="fa fa-envelope me-2 text-primary"></i>{{ $product->store->support_email }}</li>
                                            @endif
                                            @if(!empty($product->store->support_phone))
                                                <li class="mb-1"><i class="fa fa-phone me-2 text-primary"></i>{{ $product->store->support_phone }}</li>
                                            @endif
                                            @if(!empty($product->store->address))
                                                <li class="mb-1"><i class="fa fa-map-marker-alt me-2 text-primary"></i>{{ $product->store->address }}</li>
                                            @endif
                                        </ul>
                                        @if(!empty($product->store->instagram_url) || !empty($product->store->facebook_url) || !empty($product->store->twitter_url) || !empty($product->store->tiktok_url))
                                            <div class="d-flex align-items-center gap-3">
                                                @if(!empty($product->store->instagram_url))
                                                    <a href="{{ $product->store->instagram_url }}" target="_blank" class="text-secondary"><i class="fa-brands fa-instagram"></i></a>
                                                @endif
                                                @if(!empty($product->store->facebook_url))
                                                    <a href="{{ $product->store->facebook_url }}" target="_blank" class="text-secondary"><i class="fa-brands fa-facebook"></i></a>
                                                @endif
                                                @if(!empty($product->store->twitter_url))
                                                    <a href="{{ $product->store->twitter_url }}" target="_blank" class="text-secondary"><i class="fa-brands fa-twitter"></i></a>
                                                @endif
                                                @if(!empty($product->store->tiktok_url))
                                                    <a href="{{ $product->store->tiktok_url }}" target="_blank" class="text-secondary"><i class="fa-brands fa-tiktok"></i></a>
                                                @endif
                                            </div>
                                        @endif
                                    </div>
                                ...
                                @endif
                                <div class="card m-b30" style="display:none;">
                                    <div class="card-body">
                                ...
                                (function(){
                                  ...
                                  var priceWasEl = document.getElementById('deliveryPriceWas');
                                  var priceWasWrap = document.getElementById('deliveryPriceWasWrap');
                                  var hasDiscount = {{ !empty($hasDiscount) ? 'true' : 'false' }};
                                  var baseAmt = {{ isset($baseAmount) ? (float)$baseAmount : 0 }};
                                  var discAmt = {{ isset($discountedAmount) && $discountedAmount !== null ? (float)$discountedAmount : 0 }};
                                  var priceAmt = hasDiscount ? discAmt : baseAmt;
                                  var sym = @json($symbolForProduct ?? '₦');
                                  var vatPct = {{ isset($vatPercentage) ? (float)$vatPercentage : 0 }};
                                  ...
                                  function refreshAreas(){
                                    ...
                                    if (!list.length){
                                      ...
                                      opt0.value = '';
                                      opt0.textContent = 'No areas available';
                                      opt0.disabled = true;
                                      opt0.selected = true;
                                      areaSel.appendChild(opt0);
                                    ...
                                    list.forEach(function(a, idx){
                                      ...
                                      opt.setAttribute('data-fee', a.fee);
                                      opt.setAttribute('data-days', a.days);
                                  ...
                                  function updateDeliveryInfo(){
                                    var opt = areaSel && areaSel.options[areaSel.selectedIndex];
                                    if (!opt || !opt.getAttribute('data-fee')){ if(infoWrap) infoWrap.style.display='none'; return; }
                                    var feeKobo = parseInt(opt.getAttribute('data-fee') || '0', 10) || 0;
                                    var days = opt.getAttribute('data-days') || '—';
                                    var fee = feeKobo/100.0;
                                    var vat = (priceAmt * vatPct) / 100.0;
                                    var total = priceAmt + vat + fee;
                                    if (priceEl) priceEl.textContent = fmt(priceAmt);
                                    if (priceWasEl && priceWasWrap){
                                      if (hasDiscount){ priceWasWrap.style.display=''; priceWasEl.textContent = fmt(baseAmt); }
                                      else { priceWasWrap.style.display='none'; }
                                    }
                                ...
                                <a href="#" class="btn btn-secondary w-100" data-add-to-cart data-product-id="{{ $product->id }}" data-store="{{ $product->store->slug }}" data-qty-selector="{{ !empty($product->has_variants) ? '#qty-lg' : '#qty-single' }}">ADD TO CART</a>
                            </div>	
                        </div>
                    </div>
                </div>
                        ...
                        <div class="tab-pane fade" id="profile-tab-pane" role="tabpanel" aria-labelledby="profile-tab" tabindex="0">
                            <div class="clear" id="comment-list">
                                <div class="post-comments comments-area style-1 clearfix">
                                    <h4 class="comments-title mb-2">Coming soon</h4>
                ...
                <div class="swiper swiper-four">
                    <div class="swiper-wrapper">
                        <div class="swiper-slide">
                            <div class="shop-card">
                                <div class="dz-media">
                                    <img src="{{ asset('home/images/shop/product/5.png') }}" alt="image">
                                    <div class="shop-meta">
                        ...
                        <div class="swiper-slide">
                            <div class="shop-card">
                                <div class="dz-media">
                                    <img src="{{ asset('home/images/shop/product/6.png') }}" alt="image">										
                                    <div class="shop-meta">
                                        <a href="javascript:void(0);" class="btn btn-secondary btn-icon" data-bs-toggle="modal" data-bs-target="#exampleModal">
                                            <i class="fa-solid fa-eye d-md-none d-block"></i>
                                            <span class="d-md-block d-none">Quick View</span>
                        ...
                        <div class="swiper-slide">
                            <div class="shop-card">
                                <div class="dz-media">
                                    <img src="{{ asset('home/images/shop/product/7.png') }}" alt="image">	
                                    <div class="shop-meta">
                                        <a href="javascript:void(0);" class="btn btn-secondary btn-icon" data-bs-toggle="modal" data-bs-target="#exampleModal">
                                            <i class="fa-solid fa-eye d-md-none d-block"></i>
                                            <span class="d-md-block d-none">Quick View</span>
                        ...
                        <div class="swiper-slide">
                            <div class="shop-card">
                                <div class="dz-media">
                                    <img src="{{ asset('home/images/shop/product/8.png') }}" alt="image">
                                    <div class="shop-meta">
                                        <a href="javascript:void(0);" class="btn btn-secondary btn-icon" data-bs-toggle="modal" data-bs-target="#exampleModal">
                                            <i class="fa-solid fa-eye d-md-none d-block"></i>
                                            <span class="d-md-block d-none">Quick View</span>
...
<script>
    (function(){
        ...
        const qtyLg = document.getElementById('qty-lg');
        const qtySm = document.getElementById('qty-single'); // Fixed ID reference
        ...
        const matrix = @json($variantMatrix ?? []);
        const defaults = @json($defaultSelection ?? ['size' => null, 'weight' => null, 'color' => null]);
        
        // Bulk Logic
        const singleBtn = document.getElementById('buy_single');
        const bulkBtn = document.getElementById('buy_bulk');
        const bulkQty = {{ $product->bulk_quantity ?? 0 }};
        const bulkPriceHtml = '{!! $product->currency ? ($product->currency->symbol . number_format($product->bulk_price, 2)) : "" !!}';
        
        let current = { size: defaults.size, weight: defaults.weight, color: defaults.color };
        let originalPriceHtml = priceEl ? priceEl.innerHTML : '';

        function keyFor(sel){ return (sel.size ?? '') + '|' + (sel.weight ?? '') + '|' + (sel.color ?? ''); }
        
        function updateUI(){
            const isBulk = bulkBtn && bulkBtn.checked;
            const targetQty = qtyLg || qtySm;

            // Update Price
            if (isBulk) {
                if(priceEl) priceEl.innerHTML = bulkPriceHtml;
            } else {
                // Restore normal price (variant or base)
                const k = keyFor(current); 
                const data = matrix[k];
                if(data){
                    if(priceEl) priceEl.innerHTML = data.price; // Variant price
                    if(stockEl) stockEl.textContent = data.qty > 0 ? (`In stock: ${data.qty}`) : 'Out of stock';
                    if(targetQty) targetQty.max = Math.max(data.qty, 1);
                } else {
                    // No variant selected or no variants
                    if(priceEl) priceEl.innerHTML = originalPriceHtml; // Base price
                    if(stockEl) stockEl.textContent = ''; // Clear stock if not variant
                }
            }
            ...
            if(targetQty) {
                if(isBulk) {
                    targetQty.min = bulkQty;
                    if(parseInt(targetQty.value) < bulkQty) targetQty.value = bulkQty;
                } else {
                    targetQty.min = 1;
                }
            }
        ...
        function bind(sel, key){ if(!sel) return; sel.addEventListener('change', function(){ current[key] = this.value || null; updateUI(); }); }
        bind(sizeSel, 'size'); bind(weightSel, 'weight'); bind(colorSel, 'color');
        
        if(singleBtn && bulkBtn) {
            singleBtn.addEventListener('change', updateUI);
            bulkBtn.addEventListener('change', updateUI);
        }
        
        const targetQty = qtyLg || qtySm;
        if(targetQty) {
            targetQty.addEventListener('change', function(){
                if(bulkBtn && bulkBtn.checked && parseInt(this.value) < bulkQty) {
                    this.value = bulkQty;
                }
            });
        }

        // Initialize from select values if present
        if(sizeSel) current.size = sizeSel.value || null;
        if(weightSel) current.weight = weightSel.value || null;
        if(colorSel) current.color = colorSel.value || null;
        
        // Save original price if variants exist, otherwise originalPriceHtml is already set
        // Actually, if variants exist, originalPriceHtml might be the range or default.
        // But updateUI handles restoring variant price.
