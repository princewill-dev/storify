...
<div>
    <form action="{{ route('management.roles.store') }}" method="POST" class="space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <x-management.form-input name="name" label="Role Name" :value="old('name')" placeholder="Store Manager" required :error="$errors->first('name')" />
            <x-management.form-input name="description" label="Description" :value="old('description')" placeholder="Brief description of this role..." />
        </div>
        ...
        <div class="flex items-center gap-3 pt-2">
            <button type="submit" class="inline-flex items-center gap-1.5 px-5 py-2.5 bg-slate-900 text-white text-sm font-semibold rounded-lg hover:bg-slate-800 transition-colors">Create Role</button>
