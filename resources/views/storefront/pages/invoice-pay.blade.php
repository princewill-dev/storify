...
@php
    $historyTxs = $invoice->transactions->where('status', '!=', 'pending')->sortByDesc('created_at');
    $hasHistory = $historyTxs->isNotEmpty();
@endphp
<div class="w-full {{ $hasHistory ? 'max-w-5xl' : 'max-w-2xl' }}">
    ...
    <div class="grid grid-cols-1 {{ $hasHistory ? 'lg:grid-cols-5 gap-6' : '' }}">
        {{-- Invoice Card (Left) --}}
        <div class="{{ $hasHistory ? 'lg:col-span-3' : '' }}">
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden sticky top-4">
                <div class="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
                    <p class="text-xs font-semibold text-slate-400 uppercase">Payment History</p>
                    <span class="text-xs text-slate-300">{{ $historyTxs->count() }} {{ Str::plural('payment', $historyTxs->count()) }}</span>
                </div>
                <div class="divide-y divide-slate-50">
                    @foreach($historyTxs as $tx)
                    <div class="px-5 py-4">
                        <div class="flex items-start justify-between mb-2">
                            <div>
                                <p class="text-xs text-slate-400">{{ $tx->created_at->format('M d, Y') }}</p>
                                <p class="text-[10px] text-slate-300">{{ $tx->created_at->format('h:i A') }}</p>
                            </div>
                            <span class="inline-flex items-center gap-1 text-[11px] font-medium px-2 py-0.5 rounded-full {{ $tx->status->value === 'confirmed' ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-600' }}">
                                <span class="w-1.5 h-1.5 rounded-full {{ $tx->status->value === 'confirmed' ? 'bg-emerald-500' : 'bg-red-400' }}"></span>
                                {{ $tx->status->value === 'confirmed' ? 'Paid' : $tx->status->label() }}
                            </span>
                        </div>
                        <p class="font-bold text-slate-800 text-base mb-2.5">₦{{ number_format($tx->amount, 2) }}</p>
                        <div class="flex items-center gap-1.5 bg-slate-50 rounded-lg px-2.5 py-1.5">
                            <span class="font-mono text-[11px] text-slate-500 truncate flex-1" title="{{ $tx->reference }}">{{ $tx->reference }}</span>
                            <button onclick="copyToClipboard('{{ $tx->reference }}', this)" class="shrink-0 text-slate-400 hover:text-slate-600 cursor-pointer transition-colors" title="Copy reference">
                                <i class="fa-regular fa-copy text-[13px]"></i>
                            </button>
                        </div>
                        @if($tx->gateway_reference)
                        <p class="text-[10px] text-slate-300 mt-1.5 ml-1">Paystack: {{ \Illuminate\Support\Str::limit($tx->gateway_reference, 20) }}</p>
                        @endif
                    </div>
                    @endforeach
                </div>
...
function copyToClipboard(text, btn) {
    ...
    function showSuccess() {
        icon.className = 'fa-solid fa-check text-[13px] !text-emerald-500';
