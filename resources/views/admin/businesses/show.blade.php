...
<div class="flex items-center justify-between mb-6">
    ...
    <div class="flex items-center gap-2">
        <button onclick="editBusinessModal('{{ $user->account_id }}', '{{ $user->name }}', '{{ $user->slug }}', '{{ $user->email }}', '{{ $user->phone }}', '{{ $user->status }}')" class="inline-flex items-center gap-1.5 px-3 py-2 text-sm font-medium rounded-lg bg-slate-900 text-white hover:bg-slate-800">
            <i class="fi fi-rr-pencil text-sm"></i> Edit Owner
        </button>
        <button onclick="suspendBusiness('{{ $user->account_id }}', '{{ $business?->name ?? $user->name }}')" class="inline-flex items-center gap-1.5 px-3 py-2 text-sm font-medium rounded-lg border border-amber-200 bg-amber-50 text-amber-700 hover:bg-amber-100">
            <i class="fi fi-rr-ban text-sm"></i> Suspend
        </button>
        <button onclick="deleteBusiness('{{ $user->account_id }}', '{{ $business?->name ?? $user->name }}')" class="inline-flex items-center gap-1.5 px-3 py-2 text-sm font-medium rounded-lg border border-red-200 bg-red-50 text-red-700 hover:bg-red-100">
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div class="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
            ...
            <a href="{{ route('admin.stores.index') }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-50">Manage Stores</a>
        </div>
        <div class="px-5 py-4">
...
<div class="mt-6 bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
    ...
    <div class="px-5 py-4">
        @if($kyc)
            ...
            <a href="{{ route('admin.business-kyc.show', $kyc) }}" class="inline-flex items-center gap-1.5 px-3 py-2 text-sm font-medium rounded-lg bg-slate-900 text-white hover:bg-slate-800">View KYC Details</a>
        @else
...
@else
{{-- Fallback: old business view (no Business record) --}}
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div class="px-5 py-4 border-b border-slate-100">
        ...
        <div class="px-5 py-3 bg-amber-50 border-t border-amber-100">
            <p class="text-sm text-amber-700">This business has no Business record yet. The old business flow is deprecated.</p>
        </div>
    </div>
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div class="px-5 py-4 border-b border-slate-100">
...
@include('admin.businesses._modals')
@endsection

@push('scripts')
<script>
function deleteBusiness(accountId, name) {
    var nameInput = document.getElementById('deleteBusinessName');
...
function editBusinessModal(accountId, name, slug, email, phone, status) {
...
function suspendBusiness(accountId, name) {
